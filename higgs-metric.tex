...
\usepackage[bibstyle=authoryear-comp,defernumbers=true,maxnames=20,uniquename=init,dashed=false,backend=biber,sorting=none]{biblatex}
...
% \begin{center}
% \small \url{http://crabdance.com/higgs-metric.pdf}
% \end{center}
...
that the holomorphic line bundle $(\det E) \otimes \cO\left(\sum_{p \in P} n_p p\right)$ over $C$ is trivial, where $n_p = \frac{1}{2\pi}\Tr m_p^\R \in \Z$.
\item $\Tr \varphi = 0$.
\end{itemize}
\end{defn}
...
\begin{equation} \label{eq:pdeg}
  \pdeg E' = \deg E' + \frac{1}{2\pi} \sum_{p \in P} \sum_{\alpha \in [0,2\pi)} \alpha \dim \Gr_\alpha E'_p.
\end{equation}
\end{defn}
...
$(\mu, 2\pi-\mu)$, for some $\mu \in (0,\pi]$.
Assume we are in the latter case.
Now suppose $E' \subset E$ is a line subbundle.
If $E'_p$ is the line 
$E_{p,\vec\lambda}$ with $\lambda^\R = \mu$, 
then the contribution
to $\pdeg E'$ from $p \in P$ is $1 - \frac{\mu}{2\pi}$;
otherwise
the contribution is $1 - \frac{\mu}{2\pi}$. 
\end{example}

\begin{remark}[Integrality of $\pdeg E$] The full bundle $E$
has 
\begin{equation}
  \pdeg E = \deg E + \frac{1}{2\pi} \sum_{p \in P} \Tr m_p \in \Z.
\end{equation}
\end{remark}

\begin{defn}[Stability of parabolic Higgs bundles]
...
\begin{prop}[Moduli space of parabolic Higgs bundles exists]
...
is $\exp(\I m^\R)$. This is one of the most concrete ways
...
more simply:
\begin{itemize}
 \item $\Gamma^\flavor$ is the free $\Z$-module on
 $K \abs{P}$ generators, identified with small clockwise
 loops around the points of $\rho^{-1}(P)$,
 \item $\Gamma_\vphi^\gauge = H_1(\Sigma_\vphi, \Z)$,
...
canonical section of $\Hom(K_C^\half, K_C^{-\half}(-P)) \otimes K_C(P) = \cO$. 
If for each $p \in P$ we set $m^\R_p = (\pi,\pi)$
...
$\zeta^{-1} Z_\gamma \notin \R$ for all $\gamma \in \Gamma_{\phi_2}$. Then the $\zeta$-trajectories determine an ideal
