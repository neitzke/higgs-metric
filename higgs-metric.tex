...
given by \eqref{eq:X-sf} is \ti{not} a holomorphic function
on $(\cM, I^\zeta)$. Nevetheless this is
true ``asymptotically'', in two different senses: as we go
to infinity in $\cM$ \ti{or} as we take $\zeta \to 0$ or
$\zeta \to \infty$. Below we will see more precisely what
we mean by this.
...
can generate new walls; this process is completely
determined by the requirement that the $\cX_\gamma$
are well defined. \fixme{cite KS}
\end{remark}
