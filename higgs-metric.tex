...
Then $\cM_\vphi$ is a torsor over the compact complex torus $\Prym (\rho: \Sigma_\vphi \to C)$. 
After choosing spin structures on 
$C$ and $\Sigma_\vphi$, we can identify $\cM_\vphi$ with the space of flat $\U(1)$-connections $\nabla$ over $\Sigma_\vphi$, equipped with a trivialization of $\det \rho_* \nabla$.
\end{prop}
...
$\zeta^{-1} Z_\gamma \notin \bbR$ for all $\gamma \in \Gamma_{\phi_2}$. Then the $\zeta$-trajectories determine a triangulation $T(\phi_2,\zeta)$ of an $(N+2)$-gon.
\fixme{show the picture in the case $N=3$}
\end{prop}
