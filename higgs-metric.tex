...
\end{itemize}
We write $C' = C \setminus P$. We require that $-\chi(C') = 2g_C + \abs{P} - 2 > 0$; equivalently, 
...
\begin{defn}[Tame data]
We say $(G,C,\vec{m})$ is \ti{tame} if there exists at least one $p \in P$
for which $m^\C_p$ is a regular semisimple element.
\end{defn}
...
\fixme{remark on irregular cases?}
...
\begin{itemize}
  \item $E$ is a holomorphic vector bundle of rank $K$ over $C$, with parabolic structure
  (complete flag) at each point $p \in P$, with parabolic weights given by $m_p^\R$ and total parabolic degree zero,
  \item $\varphi$ is a holomorphic section of $\End E \otimes K_C(P)$,
  compatible with the parabolic
  structure (the flag at each puncture is preserved by the residue), 
  with residue $m_p^\C$.
\end{itemize}
...
Thus relative to a local holomorphic trivialization of $E$
and local coordinate on $C$, $\varphi$ would
be written as a matrix of holomorphic $1$-forms: e.g. for $G = \SU(2)$, something like
\begin{equation}
...
the space of tuples $(\phi_1, \phi_2, \dots, \phi_K)$
where $\phi_n$ is a holomorphic section of $K_C(P)^n$,
and $m_p^\C$ controls the residues via the equation\footnote{Recall that for a section of $K_C^n$ the residue is the coefficient of $\frac{(\de z)^n}{z^n}$: this is an invariantly
...
\begin{equation}
  \det (\lambda - m_p^\C) = \lambda^K + \sum_{n=1}^K \lambda^{K-n} \Res_p \phi_n.
\end{equation}
If $G = \SU(K)$ then we make the same definition except that
$\phi_1 = 0$ everywhere.

We remark that $\cB$ is a complex affine space, a torsor over the complex vector space
\begin{equation}
...
\end{equation}
$\Sigma_{\vphi}$ is a $K$-fold branched covering of $C$.
\end{defn}

The branch locus of $\Sigma_{\vphi}$ is the zero locus of the discriminant $\Delta_{\vphi}$ of the equation \eqref{eq:spectral-curve}. $\Delta_\vphi$ 
is a holomorphic section of $K_C(P)^{K(K-1)}$;
e.g. for $G = \U(2)$
\begin{equation} \label{eq:discriminant-K2}
  \Delta_{\vphi} = \phi_1^2 - 4 \phi_2.
\end{equation}
$\Sigma_\vphi$ is smooth iff $\Delta_\vphi$ has only simple zeroes; in this case
...
We should emphasize that this one-dimensional example is a little 
misleading about the generic case: generally,
when $m^\C = 0$, $\cB_\sing$ is some cone inside $\cB$, and in particular
$\cB_\sing$ is not compact.


\begin{prop}[Fibers of the Hitchin map over $\cB_\reg$, crude version]
Suppose $\vphi \in \cB_\reg$. Let $T_\vphi = \pi^{-1}(\vphi)$.
Then:
\begin{itemize}
  \item If $G = \U(K)$, then $T_\vphi$ is a torsor over
$\Jac \Sigma_\vphi$.
 \item If $G = \SU(K)$, then $T_\vphi$ is a torsor over
$\Prym \Sigma_\vphi$.
\end{itemize}
\end{prop}
A more precise description would say exactly \ti{which} torsor 
$T_\vphi$ is. For example, when $G = \U(K)$ and $P = \emptyset$, we get
$T_\vphi = \Pic^{d} \Sigma_\vphi$ with $d = -K(K-1)(g_C-1)$.
