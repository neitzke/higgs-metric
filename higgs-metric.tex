...
Solving Hitchin's equations in this case is equivalent to 
finding a harmonic map from $\C$ to the hyperbolic disc whose  Hopf differential is $\phi_2$. The image of such a map
turns out to be an ideal polygon with $n+2$ vertices.
\fixme{Han-Tam-Treibergs-Wan}

\insfig{higgs-metric-15}{0.8}

The quantities $X_\gamma = \cX_\gamma(\zeta = 1)$ in this case are monomials in cross-ratios $r_{ijkl}$ of the 
vertices of the polygon.
For example, when $n=3$, we have $5$ asymptotic vertices and $\dim \cM = 2$, so the $X_\gamma$ are monomials in
