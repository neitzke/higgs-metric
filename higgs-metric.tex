...
\begin{equation} \label{eq:spectral-coeffs}
  \det(\lambda - \varphi) = \sum_{n=0}^K \lambda^{K-n} \phi_n = 0, \qquad \phi_n \in K_C(P)^n.
\end{equation}
We always have $\phi_0 = 1$.
The remaining coefficients $\vphi = (\phi_1, \dots, \phi_K)$ 
define a point of the Hitchin base:

\begin{defn}[Hitchin base]
If $G = \U(K)$,
the \ti{Hitchin base} $\cB = \cB(G,C)$ is
the space of tuples $\vphi = (\phi_1, \phi_2, \dots, \phi_K)$
where $\phi_n$ is a holomorphic section of $K_C^n$.
...
$C$ and on $\Sigma_\vphi$, we can identify $\cM_\vphi$ with the space
of flat $\U(1)$-connections over $\Sigma_\vphi$.
...
\begin{defn}[Good filtered Higgs bundle]
...
At each pole $p$ of $\phi_2$ with $\zeta^{-1} m^\bbC_p \notin \R$, 
