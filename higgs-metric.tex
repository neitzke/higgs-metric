...
Throughout these lectures we will fix data $(G,C,P,\vec m)$ as follows:
\begin{itemize}
  \item a group $G = \SU(K)$ or $\U(K)$, with diagonal subgroup $T \subset G$,
  \item a compact Riemann surface $C$,
  \item a finite subset $P \subset C$,
  \item a vector $\vec{m}_p = (m^\C_p, m^\R_p) \in \ft_\C \oplus \ft$ for each $p \in C$.
\end{itemize}
We write $C' = C \setminus P$. We require that $\chi(C') < 0$; equivalently, 
if $C$ has genus zero we require $\abs{P} \ge 3$, 
and if $C$ has genus $1$ we require $\abs{P} \ge 1$.
Sometimes we will make the additional requirement:
\begin{defn}
We say $(G,C,P,\vec{m})$ is \ti{tame} if there exists at least one $p \in P$
...
Given the data $(G, C, P, \vec m)$ there exists a beautiful moduli space, as follows:
\begin{defn}
A $(G, C, P, \vec m)$-\ti{Higgs bundle} \fixme{...}
\end{defn}
