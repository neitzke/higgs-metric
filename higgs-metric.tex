...
(note $I^{\zeta=0} = I_1, I^{\zeta = 1} = I_2, I^{\zeta = \I} = I_3$.)
...
In particular, this means that any holomorphic function of
...
in terms of the original data $(\vphi, \theta)$.
...
Claiming that it is, would be equivalent to claiming that
$(D,\varphi)$ arise as the pushforward
of a solution of the $\U(1)$ Hitchin equations
from $\Sigma_\vphi$.
This is not true, although it is true ``asymptotically'',
...
Specialize to $G = \SU(2)$.

Let $\vartheta = \arg \zeta$.

\begin{defn}[$\vartheta$-trajectories of a quadratic differential]
A \ti{$\vartheta$-trajectory} of $\phi_2$ is a connected
real $1$-manifold $p$ on $C$
such that $e^{- \I \vartheta} \sqrt \phi_2$ (with either choice of sign
for $\sqrt{\phi_2}$) is a
...
\fixme{draw foliation}

\fixme{define complexified shear coordinates}

\fixme{state their asymptotic property}

\fixme{state their analytic properties}
