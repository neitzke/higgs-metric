...
\begin{itemize}
  ...
  that gives an estimate of the leading correction
  to $\cX_\gamma$,  
\begin{equation} \label{eq:first-asymptotic-correction}
\cX_\gamma(\zeta) = \cX_\gamma^\sf(\zeta) \exp \left[ \sum_{\mu \in \Gamma} \frac{DT(\mu) \IP{\gamma,\mu}}{4 \pi \I}
  \frac{\alpha_\mu + \zeta}{\alpha_\mu - \zeta} \sqrt{\frac{\pi}{\abs{Z_\mu}}} \e^{-2 \abs{Z_\mu}} + \cdots \right].
\end{equation}
\fixme{should also give the leading correction to $g$}
\end{itemize}
...
The $\cX_\gamma$ in this case are cross-ratios of the asymptotic vertices.
