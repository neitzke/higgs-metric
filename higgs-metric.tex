...
$g$ on $\cM$ is constructed from two distinct constituents:
\begin{enumerate}
...
$\omega = \sum_{I = 1}^{n} \de Z_{A^I} \wedge \de \overline{Z}_{B_I}$.
\end{defn}
...
coordinate system in complex structure $I^\zeta$.
Instead we will construct some
other functions $\cX_\gamma(\zeta)$ which \ti{are} true 
...
these $4$ points. \fixme{write precise formula} (This definition comes essentially from
...
appearing in the middle of the figure above.
...
a double zero) emits two walls. These two 
walls are ``hyperplanes'' 
in the sense of the special \kahler structure on $\cB_\reg$:
they are of the form $Z_{\pm \mu} / \zeta \in \R_-$, where 
...
When the discriminant meets itself, there's a more interesting structure of walls emanating. 
...
one needs to study \ti{WKB spectral networks} $\cW(\vphi, \zeta)$, as defined in \cite{Gaiotto2012}.
