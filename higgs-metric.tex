...
\subsection{The \texorpdfstring{$\cX_\gamma$}{X_\gamma}}
...
Precisely \ti{which} cross-ratios we take is 
determined by a triangulation $T(\phi_2, \zeta)$ 
of an $(n+2)$-gon, constructed similarly to the
ideal triangulations $T(\phi_2, \zeta)$ in 
...
\end{itemize}
Explicit numerical results are below, where we 
define $X_1 = r_{1235}$ and $X_2 = r_{1235} r_{1345}$.
On the left we plot the values of $X_1$ computed by
both methods; on the right we plot the error in
$\log X_1$.
The agreement is extremely good (the difference
$\log(X_1^{\mathrm i}) - \log(X_1^{\mathrm f})$ never exceeds $10^{-5}$), 
which we view as evidence
that the integral equations of \S\ref{sec:integral-equations} indeed hold.
...
Dumas and I have made similar numerical calculations 
for higher degree polynomials $P(z)$, 
as well as in a slightly fancier case: Higgs bundles
