...
I will specialize to $\SU(2)$-Higgs bundles, but the
picture is similar for any reductive $G$.

\subsection{Overview}

Fix a compact Riemann surface $C$, of genus $g_C \ge 2$,
and let $\cM$ be the 
moduli space of $\SU(2)$-Higgs bundles over $C$.
...
\begin{defn}[$\SU(2)$-Higgs bundles] \label{def:higgs}
A $\SU(2)$-\ti{Higgs bundle} is a pair $(E, \varphi)$, where:
\begin{itemize}
...
\begin{defn}[Stability of $\SU(2)$-Higgs bundles]
We say $(E,\varphi)$ is \ti{stable} if for all $E' \subset E$ preserved
by $\varphi$ we have $\deg E' < 0$.
We say $(E,\varphi)$ is \ti{strictly polystable} if it is a direct sum of
two degree zero line bundles which are both preserved by $\varphi$.
...
polystable $\SU(2)$-Higgs bundles $(E,\varphi)$ 
up to equivalence. 
...
We define a 2-form $\omega \in \Omega^{1,1}(\cB_\reg)$ by
\begin{equation}
  \omega = \DIP{\de Z, \de \overline{Z}} = \sum_{I = 1}^{r} \de Z_{A^I} \wedge \de \overline{Z}_{B_I} \, .
\end{equation}
\end{defn}
...
We should give at least one example of an irregular Higgs bundle:
...
$\zeta^{-1} Z_\gamma \notin \bbR$ for all $\gamma \in \Gamma_{\phi_2}$. Then the $\zeta$-trajectories determine an ideal
