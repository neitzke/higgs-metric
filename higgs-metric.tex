...
and a phase $\vartheta$ (think of $\vartheta$ as $\arg \zeta$.)
...
A \ti{$\vartheta$-trajectory} of $\phi_2$ is a path on $C$
along which
$\e^{- \I \vartheta} \sqrt \phi_2$ (with either choice of sign
for $\sqrt{\phi_2}$) is a
...
$\vartheta$-trajectories are the leaves of a singular
...
$\vartheta$-trajectories]
Suppose $\vartheta$ is generic, in the sense that
$\e^{\I \vartheta} Z_\gamma \notin \R$ for all $\gamma \in \Gamma_{\phi_2}$. Then the $\vartheta$-trajectories determine an ideal
triangulation $T(\phi_2,\vartheta)$ of $C$, by the picture below.
\end{prop}
...
puncture along a leaf of $T(\phi_2,\vartheta)$.
...
For example, in 4-punctured sphere case, we can find
a domain where there are jumps along exactly
$24$ rays. (In ``wildly ramified'' examples it can be
even simpler: can get jumps along just $2$ rays.)

Now we conjecture:
\begin{equation}
  \cX_\gamma(\zeta) = \cX_\gamma^\sf(\zeta) \exp \left[ -\frac{1}{4\pi \I} \sum_\mu DT(\mu) \IP{\gamma,\mu} \int_{Z_\mu \R_-} \frac{\de \zeta'}{\zeta'} \frac{\zeta' + \zeta}{\zeta'-\zeta} \log(1 - \sigma(\mu) \cX_\mu(\zeta')) \right]
\end{equation}
where $\cX_\gamma^\sf(\zeta) = \exp \left( \zeta^{-1} Z_\gamma + \zeta \overline{Z}_\gamma \right) $

\fixme{higher rank case}
